...
public class CameraFollow : MonoBehaviour
{
    ...
    VehicleSpawning FollowShipOnSpawn;
    Vehicle VehiclePos;
    ...
    void Start()
    {
        FollowShipOnSpawn = GetComponent<VehicleSpawning>();
        VehiclePos = GetComponent<Vehicle>();
    ...
    void LateUpdate()
    {
        if (FollowShipOnSpawn.VehicleCount >= 1)
        {
            //targetNumber = VehiclePos.VehicleNumber; use later
            if (target)
            {
                if (smooth)
                {
